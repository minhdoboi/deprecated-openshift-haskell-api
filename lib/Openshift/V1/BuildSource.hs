...
data BuildSource = BuildSource
    { type_ :: String -- ^ type of build input to accept 
    ...
    , dockerfile :: Maybe String -- ^ the contents of a Dockerfile to build; FROM may be overridden by your strategy source, and additional ENV from your strategy will be placed before the rest of the Dockerfile stanzas 
    ...
    , contextDir :: Maybe String -- ^ specifies sub-directory where the source code for the application exists, allows for sources to be built from a directory other than the root of a repository 
